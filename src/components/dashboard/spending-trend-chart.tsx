        ...
        {hasData ? (
          <ChartContainer config={chartConfig} className="h-87.5 w-full">
            <BarChart data={chartData} accessibilityLayer>
              ...
              <XAxis dataKey="period" tickLine={false} axisLine={false} tick={{ fontSize: 12 }} className="text-muted-foreground" />
              <YAxis tickLine={false} axisLine={false} tick={{ fontSize: 12 }} tickFormatter={formatChartCurrency} className="text-muted-foreground" />
              <ChartTooltip content={<ChartTooltipContent formatter={tooltipFormatter} />} />
              {allCategories.map((cat) => (
                <Bar key={cat.id} dataKey={`cat_${cat.id}`} stackId="a" fill={cat.color} radius={[0, 0, 0, 0]} className="transition-opacity hover:opacity-80" />
              ))}
            </BarChart>
          </ChartContainer>
        ) : (
          <div className="flex flex-col items-center justify-center py-16 space-y-2">
            <p className="text-sm text-muted-foreground text-center">No spending data available for this period</p>
            <p className="text-xs text-muted-foreground text-center">Your spending trends will appear here once you add expenses</p>
          </div>
        )}
      </CardContent>
    </Card>
