...
>) {
    ...
    const tooltipLabel = React.useMemo(() => {
        ...
        if (labelFormatter) return <div
            className={cn("font-medium", labelClassName)}>{labelFormatter(value, payload)}</div>
...
function getPayloadConfigFromPayload(config: ChartConfig, payload: unknown, key: string) {
    ...
    const nested = typeof p.payload === "object" && p.payload !== null ? (p.payload as Record<string, unknown>) : undefined

    let configKey = key
    if (typeof p[key] === "string") configKey = p[key]
    else if (typeof nested?.[key] === "string") configKey = nested[key]
